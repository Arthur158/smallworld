  ...
  return (
    <div
      ...
      className={`p-3 mb-4 border rounded ${
        isActive ? 'border-[#8B4513] bg-[#FAEBD7]' : 'border-[#5F4B32] bg-[#FAF0E6]'
      }`}
    >
...
export default function OpponentsListAll() {
    ...
    <div className="flex flex-col w-full h-full overflow-hidden border border-[#5F4B32] rounded bg-[#FDF5E6] p-4">
      <div className="flex-1 overflow-y-auto pr-2">
        {allPlayers.map((player) => {
          const isThisActive = activePlayer && activePlayer.name === player.name;
          return renderOneOpponent(player, isThisActive);
        })}
      </div>
    </div>
  );
