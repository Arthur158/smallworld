...
var TraitMap = map[Trait]TraitValue{
    ...
    "Adventurous": {Transform: func(t *Tribe) {
        t.countExtrapointsMap["Adventurous"] = func(gs *GameState) int {
            total := 0
            for _, power := range gs.Powers {
                if power.Owner == t.Owner {
                    total += 1
                }
            }
            return total
    ...
    "Fisher": {Transform: func(t *Tribe) {
        t.countExtrapointsMap["Fisher"] = func(gs *GameState) int {
            if !t.IsActive {
                return 0
            }
            total := 0
            for _, tile := range gs.TileList {
                if tile.Biome == River || tile.CheckPresence() == None || !tile.OwningTribe.checkPresence(tile, t.Race) {
                    continue
                }
                for _, neighbour := range tile.AdjacentTiles {
                    if neighbour.Biome == River || neighbour.CheckPresence() == None || !neighbour.OwningTribe.checkPresence(neighbour, t.Race) {
                        continue
                    }
                    for _, neighbour2 := range neighbour.AdjacentTiles {
                        if neighbour2.Biome == River {
                            total += 1
                            break
                        }
                    }
    ...
    "Flocking": {Transform: func(t *Tribe) {
        t.countExtrapointsMap["Flocking"] = func(gs *GameState) int {
            if !t.IsActive {
                return 0
            }
            tribeTiles := []*Tile{}
            for _, tile := range gs.TileList {
                if tile.CheckPresence() != None && tile.OwningTribe.checkPresence(tile, t.Race) {
                    tribeTiles = append(tribeTiles, tile)
                }
            }
            ...
            // Perform BFS from the first tile
            visited := make(map[*Tile]bool)
            queue := []*Tile{tribeTiles[0]}
            visited[tribeTiles[0]] = true
            
            for len(queue) > 0 {
                curr := queue[0]
                queue = queue[1:]
                
                for _, neigh := range curr.AdjacentTiles {
                    if neigh.CheckPresence() != None && 
                       neigh.OwningTribe.checkPresence(neigh, t.Race) && 
                       !visited[neigh] {
            ...
            // Check if all tribe tiles were visited
            for _, tile := range tribeTiles {
                if !visited[tile] {
                    return 0
                }
            }
    ...
    "Quarreling": {Transform: func(t *Tribe) {
        t.countExtrapointsMap["Quarreling"] = func(gs *GameState) int {
            if !t.IsActive {
                return 0
            }
            total := 0
            ...
            for _, tile := range gs.TileList {
                ...
                for len(visited) > 0 {
                    ...
                    for _, neigh := range curr.AdjacentTiles {
                        if neigh.CheckPresence() == None || !neigh.OwningTribe.checkPresence(neigh, t.Race) {
                            continue
                        }
                        alreadyFound := false
                        for _, foundTile := range founds {
    ...
    "Martyr": {Transform: func(t *Tribe) {
        t.countDefenseMap["Martyr"] = func(tile *Tile, p *Player, gs *GameState) (int, int, int, error) {
            g := 0
            if t.IsActive {
                g += 1
            }
    ...
    "Mining": {Transform: func(t *Tribe) {
        t.countPointsMap["Mining"] = func(tile *Tile) int {
            if !t.IsActive {
                return 0
            }
            count := 0
    ...
    "Muddy": {Transform: func(t *Tribe) {
        t.countPointsMap["Muddy"] = func(tile *Tile) int {
            if tile.Biome == Swamp {
                return 1
            }
            return 0
    ...
    "Mystic": {Transform: func(t *Tribe) {
        t.countPointsMap["Mystic"] = func(tile *Tile) int {
            count := 0
            if t.IsActive {
                for _, attr := range tile.Attributes {
                    if attr == Magic {
    ...
    "Stone": {Transform: func(t *Tribe) {
        t.countPointsMap["Stone"] = func(tile *Tile) int {
            if t.IsActive && tile.Biome == Mountain {
                return 1
            }
            return 0
        }
    }, Count: 4},
    "Shield": {Transform: func(t *Tribe) {
        t.startRedeploymentTileMap["Shield"] = func(tile *Tile, gs *GameState) {
            if tile.Biome == Forest {
                t.Owner.PieceStacks = AddPieceStacks(t.Owner.PieceStacks, []PieceStack{{Type: "Mushroom Shield", Amount: 1}})
            }
        }
        t.canBeRedeployedInMap["Shield"] = func(b bool, t *Tile, s string, gs *GameState) bool {
            if s == "Mushroom Shield" {
                return true
            }
            return b
        }
        t.IsStackValidMap["Shield"] = func(s string) bool {
            return s == "Mushroom Shield"
        }
        t.countRemovableAttackingStacksMap["Shield"] = func(oldStacks []PieceStack, p *Player) []PieceStack {
            for _, stack := range p.PieceStacks {
                if stack.Type == "Mushroom Shield" {
                    oldStacks = append(oldStacks, stack)
                }
            }
            return oldStacks
        }
        t.clearTileMap["Shield"] = func(tile *Tile, gs *GameState, pk int) {
            for i, stack := range tile.PieceStacks {
                if stack.Type == "Mushroom Shield" {
                    tile.PieceStacks = append(tile.PieceStacks[:i], tile.PieceStacks[i+1:]...)
                    return
        ...
        t.countDefenseMap["Shield"] = func(tile *Tile, p *Player, gs *GameState) (int, int, int, error) {
            for _, stack := range tile.PieceStacks {
                if stack.Type == " Mushroom Shield" {
                    return stack.Amount, 0, 0, nil
                }
            }
            return 0, 0, 0, nil
        }
    }, Count: 3},
    "Wise": {Transform: func(t *Tribe) {
        t.countExtrapointsMap["Wise"] = func(gs *GameState) int {
            if !t.IsActive && gs.IsTribePresentOnTheBoard(t.Race) {
                return 2
            }
            return 0
        }
    }, Count: 4},
    "Vampire": {Transform: func(t *Tribe) {
        t.countRemovablePiecesMap["Vampire"] = func(oldStacks []PieceStack, tile *Tile) []PieceStack {
            for _, stack := range tile.PieceStacks {
                if stack.Type == "Vampire" {
                    oldStacks = append(oldStacks, stack)
                }
            }
            return oldStacks
        }
        t.IsStackValidMap["Vampire"] = func(s string) bool {
            return s == "Vampire"
        }
        t.specialConquestMap["Vampire"] = func(gs *GameState, tile *Tile, stackType string) (bool, error) {
            if stackType != "Vampire" {
                return false, nil
            }
            ...
            tile.OwningTribe = t
            t.Owner.PieceStacks, _ = SubtractPieceStacks(t.Owner.PieceStacks, []PieceStack{{Type: "Vampire", Amount: 1}})
            return true, nil
        }
        t.countRemovableAttackingStacksMap["Vampire"] = func(oldStacks []PieceStack, p *Player) []PieceStack {
            for _, stack := range p.PieceStacks {
                if stack.Type == "Vampire" {
                    oldStacks = append(oldStacks, stack)
                }
        ...
        t.giveInitialStacksMap["Vampire"] = func() []PieceStack {
            return []PieceStack{{Type: "Vampire", Amount: 1}}
        }
        t.getStacksForConquestTurnMap["Vampire"] = func(player *Player, gs *GameState) {
            if !t.IsActive {
                return
            }
            newstacks := []PieceStack{}
            ...
            newstacks = append(newstacks, PieceStack{Type: "Vampire", Amount: 1})
            player.PieceStacks = newstacks
        }
    }, Count: 5},
    "Vanishing": {Transform: func(t *Tribe) {
        t.goIntoDeclineMap["Vanishing"] = func(gs *GameState) {
            for _, tile := range(gs.TileList) {
                if tile.CheckPresence() != None && tile.OwningTribe.checkPresence(tile, t.Race) {
                    t.clearTile(tile, gs, 1000)
                    t.Owner.CoinPile += 2
                }
            }
        }
    }, Count: 5},
    "Thieving": {Transform: func(t *Tribe) {
        t.countExtrapointsMap["Thieving"] = func(gs *GameState) int {
            indices := []int{}
            for _, tile := range(gs.TileList) {
                if tile.CheckPresence() != None && tile.OwningTribe.checkPresence(tile, t.Race) {
                    for _, tile2 := range(tile.AdjacentTiles) {
                        if tile2.CheckPresence() == Active && tile2.OwningTribe.Owner != t.Owner {
                            found := false 
                            for _, index := range(indices) {
                                if index == tile2.OwningTribe.Owner.Index {
                                    found = true
                                }
                            }
                            if !found {
                                indices = append(indices, tile2.OwningTribe.Owner.Index)
                            }
                        }
                    }
                }
            ...
            total := 0
            for _, index := range(indices) {
                gs.Players[index].CoinPile -= 1
                total += 1
            }
            return total
        }
    }, Count: 4},
    "Reborn": {Transform: func(t *Tribe) {
        t.getStacksForConquestTurnMap["Reborn"] = func(p *Player, gs *GameState) {
            if t.IsActive {
                return
            }
            for i := range p.PieceStacks {
                if p.PieceStacks[i].Type == "Phoenix" {
                    p.PieceStacks = append(p.PieceStacks[:i], p.PieceStacks[i+1:]...)
                    break
                }
            }
            t.Owner.PieceStacks = append(t.Owner.PieceStacks, PieceStack{Type: "Phoenix", Amount: 2})
        }
        t.IsStackValidMap["Reborn"] = func(stackType string) bool {
            return (stackType == "Phoenix" && !t.IsActive)
        }
        t.specialConquestMap["Reborn"] = func(gs *GameState, tile *Tile, stackType string) (bool, error) {
            if stackType != "Phoenix" {
                return false, nil
            }

            if gs.TurnInfo.Phase != TileAbandonment && gs.TurnInfo.Phase != DeclineChoice {
                return true, fmt.Errorf("You are supposed to sacrifice your pawns at the start of your turn!")
            }

            if t.IsActive {
                return true, fmt.Errorf("You are not in decline what the hell")
            }
            
            if t.Owner.ActiveTribe == nil {
                return true, fmt.Errorf("You do not have an active tribe!")
            }
            ...
            tile.handleAfterConquest(gs, t.Owner.ActiveTribe)

            t.Owner.PieceStacks, _ = SubtractPieceStacks(t.Owner.PieceStacks, []PieceStack{{Type: "Phoenix", Amount: 1}})
            t.clearTile(tile, gs, 1000)

            tile.OwningTribe = t.Owner.ActiveTribe
            tile.PieceStacks = AddPieceStacks(tile.PieceStacks, t.Owner.ActiveTribe.countNewTileStacks([]PieceStack{{Type: string(t.Owner.ActiveTribe.Race), Amount: 1}}, tile, gs))
            
            gs.Messages = append(gs.Messages, Message{Content: fmt.Sprintf("The reborn %s just transformed one of their tile to %s!",t.Race, t.Owner.ActiveTribe.Race)})
    ...
    "Frightened": {Transform: func(t *Tribe) {
        t.countPointsMap["Frightened"] = func(tile *Tile) int {
            for _, stack := range(tile.PieceStacks) {
                if stack.Type == string(t.Race) {
                    if stack.Amount >= 3 {
                        return 1
